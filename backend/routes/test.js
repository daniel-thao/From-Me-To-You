...
router.post("/laterDateAssociate", async (req, res) => {
  const dataArr = []
  const findUser = await db.ProductTest.findOne({ where: { id: 1 } });
  const findPost = await db.TagTest.findOne({ where: { id: 1 } });
...
*/
  ...
  // THIS ONLY WORKS IF YOU HAVE AN O:M OR A N:M relationship
  findUser.addTagTest(findPost);

  dataArr.push(findUser);
  dataArr.push(findPost);
